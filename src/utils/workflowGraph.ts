...
function readSeparateNodeData(node: WorkflowGraphNode) {
  ...
  const overlapSize = typeof node.data.overlapSize === 'number' && Number.isFinite(node.data.overlapSize)
    ...
    : (isRecord(node.data.inferenceParams) && typeof node.data.inferenceParams.overlap_size === 'number' && Number.isFinite(node.data.inferenceParams.overlap_size)
      ? Number(node.data.inferenceParams.overlap_size)
      : null)
  return {
...
function saveOutputMap(definition: WorkflowGraphDefinition) {
  const entries = definition.graph.nodes
    .filter(node => node.type === 'save_outputs')
    .flatMap((node) => {
      const outputs = isRecord(node.data.outputs) ? node.data.outputs as Record<string, unknown> : {}
      return Object.entries(outputs)
    })
  return Object.fromEntries(entries.map(([key, value]) => [key, String(value || '').trim()]).filter(([, value]) => Boolean(value)))
}

export function compileWorkflowDefinitionForRuntime(
  definition: unknown,
  overrides?: {
    outputFormat?: string
    inferenceDefaults?: Record<string, unknown>
  },
): Record<string, unknown> {
  const graphDefinition = readWorkflowGraphDefinition(definition)
  const saveNodeIds = workflowGraphSaveNodeIds(graphDefinition)
  const saveEdgesFromUtility = graphDefinition.graph.edges.find((edge) => {
    if (!saveNodeIds.has(edge.target.nodeId)) return false
    const sourceNode = graphDefinition.graph.nodes.find(node => node.id === edge.source.nodeId)
    return Boolean(sourceNode && ['load_audio_batch', 'audio_ensemble', 'audio_invert_phase', 'audio_normalize'].includes(sourceNode.type))
  })
  if (saveEdgesFromUtility) {
    const sourceNode = graphDefinition.graph.nodes.find(node => node.id === saveEdgesFromUtility.source.nodeId)
    throw new Error(`Workflow compile failed: utility node "${sourceNode?.id || saveEdgesFromUtility.source.nodeId}" output cannot be translated into legacy v1 workflow output yet`)
  }
  const stepNodes = sortWorkflowGraphStepNodes(graphDefinition)
  const runtimeIdMap = new Map(stepNodes.map((node, index) => [node.id, `step_${index + 1}`]))
  const saveOutputs = saveOutputMap(graphDefinition)
  const steps = stepNodes.map((node) => {
    const data = readSeparateNodeData(node)
    const inputEdge = graphDefinition.graph.edges.find(edge => edge.target.nodeId === node.id && edge.target.portId === 'input')
    let input = 'input'
    if (inputEdge) {
      const sourceNode = graphDefinition.graph.nodes.find(item => item.id === inputEdge.source.nodeId)
      if (!sourceNode) {
        input = 'input'
      } else if (sourceNode.type === 'input_audio') {
        input = 'input'
      } else if (sourceNode.type === 'separate' && inputEdge.source.portId.startsWith('stem:')) {
        const stem = inputEdge.source.portId.slice('stem:'.length)
        const runtimeSourceId = runtimeIdMap.get(sourceNode.id)
        if (!runtimeSourceId) {
          throw new Error(`Workflow compile failed: missing runtime id for ${sourceNode.id}`)
        }
        input = `${runtimeSourceId}.${stem}`
      } else if (['load_audio_batch', 'audio_ensemble', 'audio_invert_phase', 'audio_normalize'].includes(sourceNode.type)) {
        throw new Error(`Workflow compile failed: utility node "${sourceNode.id}" is not supported by the legacy v1 workflow compiler yet`)
      } else {
        throw new Error(`Workflow compile failed: unsupported edge source type ${sourceNode.type}`)
      }
    }

    const save = Object.fromEntries(data.stems.flatMap((stem) => {
      const ref = `${node.id}.${stem}`
      const dir = saveOutputs[ref]
      if (dir) return [[stem, dir]] as Array<[string, string]>
      const consumedByStep = graphDefinition.graph.edges.some(edge => edge.source.nodeId === node.id && edge.source.portId === `stem:${stem}` && edge.target.portId === 'input')
      if (consumedByStep) return []
      return [[stem, safeStemDir(stem)]]
    }))

    return {
      id: runtimeIdMap.get(node.id) || node.id,
      model: data.model.trim(),
      input,
      stems: [...data.stems],
      ...(data.overlapSize !== null ? { inference_params: { overlap_size: data.overlapSize } } : {}),
      save,
    }
  })
  ...
  return {
    version: 1,
    defaults: {
      device: graphDefinition.defaults.device,
      output_format: overrides?.outputFormat || graphDefinition.defaults.output_format,
      model_dir: graphDefinition.defaults.model_dir,
      inference_params: {
        ...graphDefinition.defaults.inference_params,
        ...(overrides?.inferenceDefaults || {}),
      },
    },
    steps,
  }
