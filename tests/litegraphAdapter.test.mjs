...
let hasPymssGraph = false
try {
  execFileSync(PY, ['-c', 'import pymss.graph'], { stdio: 'ignore' })
  hasPymssGraph = true
} catch {
  // The adapter unit tests remain available without a pymss development environment.
}
...
test('litegraph serialize -> adapter -> pymss load_comfy_file (round-trip)', { skip: !hasPymssGraph }, () => {
...
test('a real comfy-mss workflow file loads into the litegraph editor and round-trips', { skip: !hasPymssGraph }, () => {
  ...
  const source = JSON.parse(readFixture('example_mss_separate'))
  const graph = new LGraph()
  graph.configure({
    nodes: source.nodes,
